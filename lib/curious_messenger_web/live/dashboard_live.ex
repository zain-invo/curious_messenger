defmodule CuriousMessengerWeb.DashboardLive do
  ...
  def mount(_, %{"current_user" => current_user}, socket) do
    current_user = current_user |> Repo.preload(:conversations)
  ...
  def handle_event("create_conversation", %{"conversation" => conversation_form},
    ...
    case Chat.create_conversation(conversation_form) do
      {:ok, _} ->
        current_user = socket.assigns.current_user |> Repo.preload(:conversations)
